...
!-----------------------------------------------------------------------
      ...
      real*4, pointer :: uampl(:,:),uphase(:,:),vampl(:,:),vphase(:,:)
      real*4, pointer :: ampl(:,:), phase(:,:), freq(:)
      real*4, pointer :: uamp38(:,:), upha38(:,:), vamp38(:,:), vpha38(:,:)
     ...
     & '2SM2','M3','L2','2MK3','K2','M8','MS4'/
      data wavespeed38/0.0,&
     & 28.9841042,30.0000000,28.4397295,15.0410686,&
...
!  -  read ADCIRC fort.54 file
      ...
      allocate(vampl(nfreq,np),vphase(nfreq,np))
      allocate(uampl(nfreq,np),uphase(nfreq,np))
      allocate(uamp38(38,np),upha38(38,np),vamp38(38,np),vpha38(38,np))
3679  format(1x,e20.10,1x,f10.7,1x,f12.8,2x,a10)
      do i=1,nfreq
        read(54,3679)freq(i),nodefactor,equilarg,freqname(i)
      ...
      do i=1,nnodes
        ...
        read(54,*) num
        do j=1,nfreq
          read(54,*) uampl(j,i),uphase(j,i),vampl(j,i),vphase(j,i)
          if (match(j)/=-1) then
            uamp38(match(j),i)=uampl(j,i)
            upha38(match(j),i)=uphase(j,i)
            vamp38(match(j),i)=vampl(j,i)
            vpha38(match(j),i)=vphase(j,i)
           endif
        end do
      end do
      close(54)
...
! Write tidal information to the netcdf file
     ...
     & -1.,-1.,uamp38,upha38,vamp38,vpha38,tide_analysis,time_analysis)
