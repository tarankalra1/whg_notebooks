...
!-----------------------------------------------------------------------
      ...
      real*4, pointer :: uamp37(:,:), upha37(:,:), vamp37(:,:), vpha37(:,:)
...
!  -  read ADCIRC fort.54 file
      ...
      allocate(uamp37(37,np),upha37(37,np),vamp37(37,np),vpha37(37,np))
3679  format(1x,e20.10,1x,f10.7,1x,f12.8,2x,a10)
      ...
      do i=1,nnodes
        ...
        do j=1,nfreq
          ...
          if (match(j)/=-1) then
            uamp37(match(j),i)=uampl(j,i)
            upha37(match(j),i)=uphase(j,i)
            vamp37(match(j),i)=vampl(j,i)
            vpha37(match(j),i)=vphase(j,i)
           endif
        end do
      end do
      close(54)
...
! Write tidal information to the netcdf file
     ...
     & -1.,-1.,uamp37,upha37,vamp37,vpha37,tide_analysis,time_analysis)
