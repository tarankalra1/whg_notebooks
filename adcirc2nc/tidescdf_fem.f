...
C Set optional variable flags
      ...
      l_phase = .TRUE.
      lv_phase = .TRUE.
c     if (u_amp(1,1,1).le.0.0) lu_amp=.FALSE.
      if (ampl(1,1).le.0.0) l_amp=.FALSE.
c     if (u_phase(1,1,1).le.0.0) lu_phase=.FALSE.
      if (phase(1,1).le.0.0) l_phase=.FALSE.
c     if (v_amp(1,1,1).le.0.0) lv_amp=.FALSE.
c     if (v_phase(1,1,1).le.0.0) lv_phase=.FALSE.
